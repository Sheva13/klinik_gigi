...
<style>
/* Styling Disesuaikan agar tetap terlihat bagus di AdminLTE (Dark Mode) */
:root {
--gold-primary: #D4AF37;
--gold-hover: #b89628;
--bg-dark: #121212;
...
/* Override AdminLTE for Dark Theme consistency */
body { background-color: var(--bg-dark); }
.content-wrapper { background-color: var(--bg-dark); }
.main-header { border-bottom: 1px solid var(--border-color) !important; }
...
<!-- Pastikan layout adminlte memuat Material Symbols, jika tidak, tambahkan ini: -->
...
<div class="edit-card">
    <div class="mb-4 border-bottom border-secondary pb-3">
        ...
        <p class="text-muted mb-0">Pasien:
            {{-- Menggunakan optional chaining untuk RekamMedis (agar tidak error jika null) --}}
            <span class="text-gold fw-bold">{{ $reservasi->rekamMedis?->nama ?? 'Nama Pasien Tidak Ditemukan' }}</span>
    ...
    <form action="{{ route('reservasi.admin.update', $reservasi->id) }}" method="POST">
                ...
                <select class="form-select form-select-dark" id="poli_id" name="poli_id">
                    ...
                    @if (isset($polis))
                        @foreach($polis as $poli)
                            @php
                                // PERBAIKAN PENTING DI SINI (LOKASI LINE 173/176):
                                // Menggunakan optional chaining (?->) pada $reservasi->dokter dan masterPoli
                                // untuk mencegah error jika dokter atau poli belum terisi (null).
                                $currentPoliCode = $reservasi->dokter?->masterPoli?->kode_poli;
                                $selected = $currentPoliCode == $poli->kode_poli ? 'selected' : '';
                            @endphp
        ...
        @php
            ...
            // Menggunakan optional chaining untuk jadwal juga
            $jamSaatIni = $reservasi->jadwal?->jam_mulai
